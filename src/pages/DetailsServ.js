...
const DetailsServ = () => {
  ...
  const username = user?.displayName?.replaceAll(' ', '')
  console.log("🚀 ~ file: DetailsServ.js ~ line 15 ~ DetailsServ ~ username", username)
  ...
  // loader added
  if (loading) {
    return <div className={`${loading ? 'visible' : 'hidden'} absolute top-16 left-0 z-50 flex items-center justify-center w-full h-screen bg-gray-900 bg-opacity-20`}>
  ...
  return (
    <div>
      ...
      <div>
        ...
        <div >
          {
            review[0]?._id ? <div className="flex flex-col gap-5">
              {
                review.map(r => {
                  ...
                  return (
                    <div key={r._id} className="flex flex-col">
                      <div className="flex gap-5">
                        <div className="w-52 h-16 rounded-full">
                        <img className="w-full ring-2 h-full rounded-full shadow" src={r?.image} alt="" />
                        </div>
                        <div className="flex flex-col"><p className="text-base font-semibold">{r?.username}</p>
                        <div className="flex gap-1 mt-3">
                        {
                          [...Array(rating)].map((rating, i) => {
                            return (
                              <div key={i} className="w-5 h-5 rounded-full bg-green-500 flex  flex-col "></div>
                            )
                          })
                            }
                          </div>
                          <div className="my-6">
                      <p className="text-base font-semibold ">{r.reviewTitle}</p>
                            <p className="mt-4 text-sm ">{r.reviewDes}</p>
                            {
                              user?.displayName === r.username ? <div className="mt-6 flex items-center gap-7">
                              <p className="font-semibold text-base hover:underline transition-colors cursor-pointer">Edit</p>
                              <p className="font-semibold text-base hover:underline transition-colors cursor-pointer">Delete</p>
                              </div> : <div className="flex items-center gap-8 text-2xl">
                                  <button className=" font-bold text-xl text-black  mt-5 rounded-full"><BsHeart></BsHeart></button>
                                  <button className=" font-bold text-xl text-black  mt-5 rounded-full"><FaRegCommentDots className="focus:bg-red-500"></FaRegCommentDots></button>
                            </div>
                            }
                      </div>
                        </div>
